...
// 控制hk相机，自动开启的node
bool hk_auto_open_flag = 1;
int main(int argc, char *argv[])
{
    if(hk_auto_open_flag)
    {
        // 另起一个终端，开启hk相机
        system("gnome-terminal -- bash -c 'roslaunch hk_camera hk_camera.launch'&");
    }
    system("rosrun image_deal static_image_to_coordinate.py");
    // system("rosnode kill /hk_camera_node");
    // 初始化节点，句柄
    ros::init(argc,argv,"static_grap_move_num");
    ros::NodeHandle nh;
    ...
    float set_coordinate_z = 50;

    std::vector<float> vector_world_pose;
    int goal_count;

    nh.getParam("/goal_count",goal_count);
    nh.getParam("/list_world_coordinate_pose",vector_world_pose);

    for(int i = 0; i<goal_count; i++)
    {
        ROS_INFO("------------------------------------------");
        // 开启第n次抓取操作，go，go，go!
        ROS_INFO("Currently, do the %d grab operation ......", i+1);

        // 信息获取
        goal_coordinate_x = vector_world_pose[i*4+0];
        goal_coordinate_y = vector_world_pose[i*4+1];
        goal_coordinate_z = vector_world_pose[i*4+2];
        goal_rotate_rz = vector_world_pose[i*4+3];
        ...
        float temp_coordinate_z = goal_coordinate_z+0.05;
        ...
        /* -----------4. 夹爪闭合------------- */
        system("rosrun dh_hand_driver hand_controller_client 1 2 50");


        /*-------------5. 机械臂上升，实现对末端关节角度的控制-------*/ 
        wait_position[2] = temp_coordinate_z;
        grap_cmd_data.request.pose = wait_position; 
        ...
        /* -----------7. 夹爪张开，放置物体------------ */
        system("rosrun dh_hand_driver hand_controller_client 1 70 90");
        /* -----------8. 机械臂回到终止位置,机械臂上升------------ */
        wait_position[2] = temp_coordinate_z + 0.05;
        grap_cmd_data.request.pose = wait_position; 
